...
class CommandStats(object):
    def __init__(self, size=CMD_STATS_SIZE):
        super(CommandStats, self).__init__()
        self.cmdList = []
        self.size = size

    def append(self, dpid, xid, stats, req):
        if len(self.cmdList) >= self.size:
                del self.cmdList[0]
        data = FlowStats(dpid, xid, stats, req)
        self.cmdList.append(data) 

    def get(self, index):
        if index < len(self.cmdList):
...
class FlowStats(object):
    def __init__(self, dpid, xid, stats, req):
    ...
    def __str__(self):
        return json.dumps({"xid": self.xid, 
                            "url": self.url, 
                            "method": self.method,
...
class StatsController(ControllerBase):
    ...
    def get_desc_stats(self, req, dpid, **_kwargs):
        ...
        if _ofctl is not None:
            ...
            # by jesse
            self.cmdStats.append(dp.id, dp.xid, "Timeout", req)
    ...
    # by jesse
    def get_cmd_stats(self, req, cmd, **_kwargs):
        if type(dpid) == str and not dpid.isdigit():
            LOG.debug('invalid dpid %s', dpid)
            return Response(status=400)
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        uri = path + '/stats/cmd/all'
        mapper.connect('stats', uri,
                       controller=StatsController, action='send_experimenter',
                       conditions=dict(method=['POST']))

        uri = path + '/stats/cmd/{xid}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='send_experimenter',
                       conditions=dict(method=['POST']))
