...
class CircleList(object):
    ...
    def append(self, flow=None, status=None):
        ...
        t = time.time()
        cmd = result(self.id, flow , status, 
                datetime.datetime.fromtimestamp(t).strftime('%Y-%m-%d %H:%M:%S')) 
    
        self.id_list.append(self.id)
        self.dict[self.id] = cmd
        return self.id

    def result(self, id=None, flow=None, status=None, time=None):
        if id == None :
            return { "id": -1, "flow": "",
                     "status": "", "time": ""}
    ...
    def update(self, id=None, status=None):
        pass

    def get(self, id=None):
        if id == None : return self.result();
        if id in self.dict:
            return self.dict[id]
        return self.result();
...
class StatsController(ControllerBase):
    ...
    def get_flow_handling_status(self, req, id, **_kwargs):
        data = self.CMD_LIST.get(id)
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        uri = path + '/flowstatus/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='get_flow_handling_status',
                       conditions=dict(method=['GET']))
    ...
    def send_barrier_request(self, dp):
